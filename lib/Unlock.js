...
var Devices = {
  HUAWEI_EMUI8: function (obj) {
    ...
    this.unlock_pattern = function (password) {
      ...
      let pattern_view = id("com.android.systemui:id/lockPatternView").findOne(this.config.timeout_findOne).bounds(),
        pattern_size = 3,
        len = password.length,
    ...
    this.unlock_pin = function (password) {
      ...
      for (let i = 0; i < password.length; i++) {
        ...
        id(key_id).findOne(this.config.timeout_findOne).click();
        sleep(100);
      }
      return this.check_unlock();
    }
...
var MyDevice = Devices.XIAOMI_MIX2S;
...
function Unlocker(automator, config) {
  ...
  this.failed = function () {
    ...
    this.reTry ++
    if (this.reTry > 3) {
      commonFunctions.log("解锁失败达到三次，停止运行");
      engines.stopAll()
    } else {
      var sleepMs = 5000 * this.reTry;
      commonFunctions.log("解锁失败，" + sleepMs +"ms之后重试")
      sleep(sleepMs)
      this.run_unlock
    }
  }
  ...
  this.run_unlock = function () {
    ...
    if (!this.is_locked()) { 
      if (this.relock === true) {
        commonFunctions.log("需要重新锁定屏幕")
      } else {
        commonFunctions.log("不需要重新锁定屏幕")
        this.relock = false
      }
      return true;
    }
    ...
    this.swipe_layer();
    // 如果有锁屏密码则输入密码
    if (this.is_passwd() && !_device.unlock(this.config.password)) {
      // 如果解锁失败
      this.failed();
    }
  }
...
function Unlock(automator, config) {
  ...
  return {
    ...
    needRelock: function () {
      commonFunctions.log("是否需要重新锁定屏幕：" + _unlocker.relock)
