...
function SimpleFloatyButton (threadName, buttons, rerender, handleExit) {
  ...
  let window = floaty.rawWindow(typeof rerender == 'function' ? rerender(btns) :
    `<horizontal>
    <vertical padding="1">
   ${btns.map(btn => {
  ...
  btns.forEach(btn => {
    ...
    if (btn.onClick) {
      window[btn.id].on('click', () => {
        if (btn.alwaysClickable) {
          threadPool.execute(function () {
            try {
              btn.onClick()
            } catch (e) {
              console.error('点击执行异常：' + e.message)
            }
          })
          return
        }
        // region 点击操作执行中，可以定义handleExecuting触发终止，等待执行结束
        if (data.clickExecuting) {
          // 当前执行中的按钮id匹配时才能触发
          if (btn.handleExecuting && data.executingBtn == btn.id) {
            threadPool.execute(function () {
