 ...
 * @Last Modified time: 2019-12-13 22:22:54
 * @Description: 
 */
...
module.exports = (() => {
  ...
  BaiduOcrUtil.prototype.getAccessToken = function () {
    let storedToken = accessTokenStorage.get('access_token')
    if (storedToken && storedToken.expires_at > new Date().getTime() && storedToken.access_token) {
      _logUtils.debugInfo(['直接从缓存中获取accessToken，到期时间: {}', formatDate(new Date(storedToken.expires_at))])
