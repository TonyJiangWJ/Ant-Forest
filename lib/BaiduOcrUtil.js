 ...
 * @Last Modified time: 2020-08-05 09:31:32
 * @Description: 
 */
...
module.exports = (() => {
  ...
  BaiduOcrUtil.prototype.recognize = function (imgBase64) {
    let start = new Date().getTime()
    let accessToken = this.getAccessToken()
    if (!accessToken) {
      _logUtils.errorInfo('获取AccessToken失败')
      return
    }
    let okHttpClient = new OkHttpClient()
  ...
  BaiduOcrUtil.prototype.recognizeGeneralText = function (imgBase64) {
    ...
    if (response != null) {
      if (response.body() != null) {
        ...
        _logUtils.debugInfo('识别请求结果：' + resultString)
        result = JSON.parse(resultString)
        _logUtils.debugInfo(['请求通用文字识别接口总耗时：{}ms', (new Date().getTime() - start)])
      }
      response.close()
    }
    return result
  ...
  BaiduOcrUtil.prototype.getDirectly = function(imgBase64) {
    ...
    if (result && result.words_result_num > 0) {
      ...
      if (filter && filter.length > 0) {
        ...
        result = parseInt(filter[0].words)
  ...
  BaiduOcrUtil.prototype.clearAllCached = function () {
