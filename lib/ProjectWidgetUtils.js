...
const ProjectWidgetUtils = function () {
  ...
  this.getYouCollectEnergyByWidget = function () {
    ...
    if (youGet) {
      ...
      let region = [bd.left - 5, bd.bottom - 5, bd.right - bd.left + 10, (bd.bottom - bd.top) * 1.5]
      let energySum = this.widgetGetOne(/\d+g/, null, null, false, matcher => matcher.boundsInside(region[0], region[1], region[0] + region[2], region[1] + region[3]))
      if (energySum) {
        if (energySum.desc()) {
  ...
  this.checkAndClickWatering0 = function () {
    // 直接通过偏移量获取浇水按钮
  ...
  this.checkAndClickWatering = function () {
    ...
    if (screen && _config.image_config.water_icon) {
    ...
    return this.checkAndClickWatering0()
  }
  ...
  this.enterCooperationPlantAndDoWatering = function () {
    ...
    if (_config.enable_watering_cooperation && _config.watering_cooperation_name && _config.image_config.watering_cooperation) {
      ...
      let screen = _commonFunctions.checkCaptureScreenPermission()
      if (screen) {
        let point = OpenCvUtil.findByGrayBase64(screen, _config.image_config.watering_cooperation)
        if (point) {
