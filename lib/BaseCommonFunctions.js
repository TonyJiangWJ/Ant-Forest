...
let SCREEN_FAILED_COUNT = 'screenFailedCount'
...
function CommonFunctions () {
  ...
  storageFactory.initFactoryByKey(SCREEN_FAILED_COUNT, { count: 0 })
  // 初始化森林相关的存储数据
  this.initStorageFactory()
  // 截图锁
  let captureScreenLock = threads.lock()
  let last_capture_screen_time = 0
  // 初始化生命周期回调 start
   ...
   * @param {boolean} restart 启用无障碍服务后是否重启脚本
   */
  this.checkAccessibilityService = function (force, restart) {
  ...
  this.ensureOtherAccessibilityEnabled = function () {
    if (!_config.other_accessisibility_services) {
      return true
    }
    let hasAdbPermission = this.hasAdbPermission()
    ...
    try {
      let enabledServices = Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES)
      _logUtils.debugInfo(['当前已启用无障碍功能的服务:{}', enabledServices])
      enabledServices = (enabledServices || '').split(':')
      let other_accessisibility_services = _config.other_accessisibility_services.split(':')
      let anyNotSet = other_accessisibility_services.filter(v => enabledServices.indexOf(v) <= -1).length > 0
      if (anyNotSet) {
        _logUtils.warnInfo(['有第三方无障碍服务未启用，重新触发ADB授权: {}', JSON.stringify(other_accessisibility_services.filter(v => enabledServices.indexOf(v) <= -1))])
        this.checkAccessibilityService(true, true)
      } else {
        _logUtils.debugInfo(['第三方无障碍服务均已启用：{}', _config.other_accessisibility_services])
      }
    } catch (e) {
      _logUtils.warnInfo(['校验第三方无障碍异常' + e])
    }
    ...
    if (!this.checkAccessibilityService(force)) {
      try {
        ...
        while (limit-- > 0 && currentPackageName !== context.getPackageName()) {
          _logUtils.debugInfo(['当前包名：{}', currentPackageName])
          sleep(1000)
      ...
      // 校验第三方应用无障碍权限
      this.ensureOtherAccessibilityEnabled()
    }
    return true
  }
  ...
  this.captureScreen = function (errorLimit, releaseLock, captureGap) {
    return this.checkCaptureScreenPermission.apply(this, arguments)
  }
   ...
   * @param {number} captureGap 截图间隔，避免截图太快
   * @param {boolean} resetPermission 是否已经重新申请过权限，避免重置失效陷入死循环
   */
  this.checkCaptureScreenPermission = function (errorLimit, releaseLock, captureGap, resetPermission) {
    this.requestScreenCaptureOrRestart()
    captureScreenLock.lock()
    if (typeof captureGap === 'undefined') {
      captureGap = _config.capture_screen_gap || 10
    } else if (typeof captureGap != 'number') {
      captureGap = 0
    }
    if (new Date().getTime() - last_capture_screen_time < captureGap) {
      // 间隔太短 可能会导致截图卡死 延迟一下
      sleep(captureGap)
    }
    try {
      ...
      if (WarningFloaty.toDrawList.length > 0 || !!_FloatyInstance.currentText) {
        // 截图可能导致将绘制的内容也截取 延迟一定时间
        sleep(100)
        _logUtils.debugForDev('延迟等待截图，避免将绘制内容也截取')
      }
      let start = new Date().getTime()
      if (!_config.async_waiting_capture) {
        ...
        do {
          ...
          if (!screen && automator.takeScreenshot) {
            _logUtils.warnInfo(['获取截图失败，尝试无障碍截图'])
          ...
          if (!screen) {
            ...
            if (errorLimit > errorCount + 1) {
              _logUtils.debugInfo(['获取截图失败，toast触发界面渲染: ' + errorCount])
              toastLog('截图失败：' + errorCount)
              sleep(1000)
            } else {
              // 滑动界面，触发渲染
              _logUtils.debugInfo('获取截图失败，尝试切换多任务界面，触发渲染')
              recents()
              sleep(1000)
              back()
            }
        ...
        if (!screen) {
          ...
          if (!resetPermission) {
            ...
            if (requestPermission) {
              return this.checkCaptureScreenPermission(1, releaseLock, captureGap, true)
            }
          }
          ...
          if (this.increaseScreenFailedCount() >= 3) {
            _logUtils.warnInfo(['获取截图失败多次，可能有其他脚本锁死截图权限，直接杀死其他脚本'])
            // 可能有其他脚本锁死截图权限，直接杀死其他脚本
            engines.all().filter(engine => engine.id !== ENGINE_ID).forEach(engine => {
              _logUtils.warnInfo(['杀死脚本：{}', engine.getSource()])
              engine.forceStop()
            })
          }
          _config.resetBrightness && _config.resetBrightness()
          _runningQueueDispatcher.removeRunningTask(true)
          _runningQueueDispatcher.executeTargetScript(this.getRunningScriptSource(), engines.myEngine().execArgv)
          exit()
        }
      }
      this.markScreenSuccess()
      _logUtils.debugInfo(['获取截图耗时：{}ms', new Date().getTime() - start])
      return screen
    } finally {
      last_capture_screen_time = new Date().getTime()
  ...
  this.minimize = function (currentPackageName, confirmCallback) {
  ...
  this.showDialogAndWait = function (checkDismissReason, notCheckUpdate, taskName) {
    ...
    if (checkDismissReason) {
      ...
      if (dismissReason) {
        ...
        // 正常执行，撤销延迟启动的通知
        NotificationHelper.cancelNotice(null, 100)
        return
      }
    }
    ...
    threads.start(function () {
      ...
      updateChecker.postCurrentVersion()
    ...
    if (continueRunning) {
      ...
      let executeNext = checkIfExecuteWaitingTask()
      _config.resetBrightness && _config.resetBrightness()
      this.setUpAutoStart(5, true)
      NotificationHelper.createNotificationWithStart('延迟五分钟后启动', '点击可直接运行')
      // 如果执行等待中的任务，则不需要延迟所有任务
      if (executeNext) {
        _runningQueueDispatcher.removeRunningTask(false, false, null, false)
      } else {
        _runningQueueDispatcher.removeRunningTask(false, false, null, true)
      }
      exit()
    }
  }

  function checkIfExecuteWaitingTask () {
    let executeNext = false
    let waitingTask = _runningQueueDispatcher.getWaitingStatus()
    if (waitingTask) {
      let lock = threads.lock()
      let complete = lock.newCondition()
      lock.lock()
      threads.start(function () {
        let newDialog = dialogs.build({
          title: '存在等待中任务，是否继续执行?',
          content: '点击继续执行将执行等待中任务，点击延迟执行，将等待五分钟后触发当前任务并执行等待中任务',
          positive: '继续执行',
          positiveColor: '#f9a01c',
          negative: '延迟执行',
          negativeColor: 'red',
  ...
  this.showMiniFloaty = function (text, x, y, color, textSize) {
    _FloatyInstance.setFloatyInfo(
      { x: x || _config.min_floaty_x || 150, y: y || _config.min_floaty_y || 20 },
      text,
      { textSize: textSize || _config.min_floaty_text_size || 8 }
    )
   ...
   * 解锁时按下音量键延迟执行
   */
  this.listenDelayBeforeUnlock = function () {
    this.preListenThread = null
    threads.start(function () {
      if (context.getSystemService(context.KEYGUARD_SERVICE).inKeyguardRestrictedInputMode()) {
        _logUtils.infoLog('即将解锁执行，按音量下键延迟五分钟执行', true)
      }
      sleep(200)
      _logUtils.debugInfo('[BeforeUnlock] after setMaxListeners')
      events.observeKey()
      _logUtils.debugInfo('[BeforeUnlock] after observeKey')
      events.onceKeyDown('volume_down', function (event) {
        _config.resetBrightness && _config.resetBrightness()
        _config.auto_lock = false
        _logUtils.warnInfo('延迟五分钟后启动脚本', true)
        _this.setUpAutoStart(5)
        NotificationHelper.createNotificationWithStart('延迟五分钟后启动脚本', '点击可直接运行')
        _runningQueueDispatcher.removeRunningTask(false, false, null, true)
        exit()
      })
   ...
   * 有非预期的反应，暂不起作用
   */
  this.cancelDelayKeyDown = function () {
    if (this.preListenThread) {
      this.preListenThread.interrupt()
      events.removeAllKeyDownListeners('volume_down')
      this.preListenThread = null
      _logUtils.debugInfo(['cancel volume down listener'])
    }
  }
  ...
  this.killRunningScript = function (forceMain) {
    ...
    let checkSource = forceMain ? mainScriptJs : engines.myEngine().getSource()
    if (runningSize > 1) {
      runningEngines.forEach(engine => {
        let compareEngine = engine
        let compareSource = compareEngine.getSource() + ''
        _logUtils.debugInfo('对比脚本信息 id:' + compareEngine.id + ' source:' + compareSource)
        if (compareSource === checkSource && compareEngine.id !== engines.myEngine().id) {
  ...
  this.setUpAutoStart = function (minutes, forceSetup) {
    ...
    // 定时任务会丢失执行参数，需要对发起脚本设置自启动
    if (_config._force_auto_start_script) {
      _logUtils.debugInfo(['设置自动启脚本为：{}', _config._force_auto_start_script])
      mainScriptJs = _config._force_auto_start_script
    }
  ...
  */
   ...
   * @param {function} messageHandler 监听消息的处理函数
   * @returns 
   */
  this.commonDelay = function (minutes, text, removeRunningTask, stopByMessage, messageHandler) {
    ...
    if (stopByMessage) {
      _logUtils.debugInfo('监听脚本执行完毕的通知', true)
      processShare.subscribe(function (str) {
        ...
        if (messageHandler && typeof messageHandler === 'function') {
          try {
            ...
            _logUtils.errorInfo('消息处理异常' + e)
            _this.printExceptionStack(e)
          }
        }
  ...
  this.requestScreenCaptureOrRestart = function (doNotRestart) {
    ...
    this.forceCheckForcegroundPermission()
    let screenOrientation = context.getResources().getConfiguration().orientation
    _logUtils.debugInfo(['当前屏幕朝向：{}', screenOrientation])
    if (screenOrientation == 0) {
      _logUtils.warnInfo(['当前屏幕为横向，为避免出现问题，回到桌面再执行截图权限请求'])
      home()
    }
    // 请求截图权限
  ...
  this.ensureDeviceSizeValid = function () {
    ...
    if (screen) {
      ...
      if (width > height) {
        _logUtils.errorInfo(['检测到截图的宽度大于高度，可能截图方法出现了问题，请尝试强制重启AutoJS，否则脚本无法正常运行! w:{} h:{}', width, height], true)
        _runningQueueDispatcher.removeRunningTask()
        _config.resetBrightness && _config.resetBrightness()
        // 设置五分钟后再次尝试，一般是因为横屏状态下导致的
        this.setUpAutoStart(5, true)
        NotificationHelper.createNotificationWithStart('检测到截图的宽度大于高度', '点击可直接运行。可能截图方法出现了问题，请尝试强制重启AutoJS，否则脚本无法正常运行!')
        exit()
      }
      if (width == 0 || height == 0) {
        _logUtils.errorInfo(['获取截图宽高异常 结果为「{}, {}」', width, height])
        return
      }
      if (width !== _config.device_width || height !== _config.device_height) {
        _logUtils.warnInfo(['设备分辨率设置不正确，宽高已修正为：[{}, {}]', width, height])
        _config.overwrite('device_height', height)
        _config.overwrite('device_width', width)
      }
    }
  }
  ...
  this.delayStartIfInSkipPackage = function (notLingering) {
    ...
    if (this.inSkipList(currentRunningPackage)) {
      ...
      if (!this.askIfStartWhenSkipedTooMuch()) {
        ...
        NotificationHelper.createNotificationWithStart('当前包名在应用白名单中，延迟5分钟执行', '点击可直接运行')
  ...
  this.delayStartIfInPayCode = function () {
    ...
    if (paycodeContent && paycodeContent.content.indexOf(regexContent) < 0) {
      ...
      this.setUpAutoStart(5)
      NotificationHelper.createNotificationWithStart('当前在支付码界面，延迟5分钟后执行', '点击可直接运行')
  ...
  this.delayIfBatteryLow = function () {
    ...
    if (!_config._triggerByNotice) {
      _logUtils.debugInfo('当前由通知触发，不做电量判断，直接执行')
      return
    }
    if (!device.isCharging() && battery < _config.battery_keep_threshold) {
      ...
      NotificationHelper.createNotificationWithStart('当前电量低，延迟一小时后启动', '点击可直接运行')
      _config.forceStop = true
      exit()
    }
  }
  ...
  this.markScreenSuccess = function () {
    this.updateRuntimeStorage(SCREEN_FAILED_COUNT, { count: 0 })
  }

  this.increaseScreenFailedCount = function () {
    let failedCount = this.getTodaysRuntimeStorage(SCREEN_FAILED_COUNT).count
    failedCount += 1
    this.updateRuntimeStorage(SCREEN_FAILED_COUNT, { count: failedCount })
    _logUtils.debugInfo(['截图失败，当前失败次数：{}', failedCount])
    return failedCount
  }
  ...
  this.checkForegroundPermition = function () {
    ...
    _logUtils.debugInfo(['授权结果: {}', result])
    if (result == 0) {
      _logUtils.infoLog('应用已获得前台服务权限')
      return true
    } else {
      _logUtils.errorInfo('应用没有前台服务权限，执行可能闪退，请授予AutoJS通知权限', true)
      return false
    }
  }


  this.openAutoJSDetailSetting = function () {
  ...
  this.openAutoJSForcegroundPermission = function () {
    let intent = new Intent()
  ...
  this.forceCheckForcegroundPermission = function () {
    if (!this.checkForegroundPermition()) {
      this.openAutoJSForcegroundPermission()
      sleep(2000)
      let timeout = 15
      while (0 != context.getPackageManager().checkPermission(android.Manifest.permission.FOREGROUND_SERVICE, context.getPackageName()) && timeout-- > 0) {
        sleep(1000)
        if (timeout == 5) {
          _logUtils.errorInfo('应用没有前台服务权限，执行可能闪退，请授予AutoJS通知权限', true)
        }
      ...
      if (!this.checkForegroundPermition()) {
        _logUtils.errorInfo('用户未授权通知权限，APP闪退或报错概不负责')
      }
    }
  }

  this.backHomeIfInVideoPackage = function () {
    let currentPackageName = currentPackage()
    _logUtils.debugInfo(['当前前台应用包名：{}', currentPackageName])
    if (_config.video_packages.filter(pkgInfo => pkgInfo.packageName == currentPackageName).length > 0) {
      _logUtils.warnInfo(['当前前台应用为视频APP 需要回到桌面'])
      home()
      sleep(500)
    }
  }
