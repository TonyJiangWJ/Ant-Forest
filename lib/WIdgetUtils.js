...
const loadFriendList = function () {
  ...
  let friendListCheckThread = threads.start(function () {
    while (widgetCheck(config.get('friend_list_ui_content'), 500)) {
      sleep(50)
    }
    warnInfo('当前所在位置非好友排行榜，直接退出等待')
    countDown.countDown()
  })
  let timeoutThread = threads.start(function () {
  ...
  friendListCheckThread.interrupt()
