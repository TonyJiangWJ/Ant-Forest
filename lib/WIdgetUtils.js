...
const homePageWaiting = function () {
  if (widgetCheck(config.get('friend_home_ui_content'), 500)) {
    errorInfo('错误位置：当前所在位置为好友首页')
    return false;
  }
  if (widgetCheck(config.get('friend_list_ui_content'), 500)) {
    errorInfo('错误位置：当前所在位置为好友排行榜')
    return false;
  }
  return widgetWaiting(config.get('home_ui_content'), '个人首页')
}
...
const friendHomeWaiting = function () {
  return widgetWaiting(config.get('friend_home_ui_content'), '好友首页')
}
...
const friendListWaiting = function () {
  return widgetWaiting(config.get('friend_list_ui_content'), '好友排行榜')
}
...
const waitRankListStable = function () {
  ...
  let size = config.get("friendListStableCount") || 5
