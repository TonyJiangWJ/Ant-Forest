...
function UpdateDownloader (github_latest_url, gitee_relase_url, gitee_package_prefix, gitee_package_url) {
  ...
  this.init = function () {
    ...
    resolver()
    runtime.loadDex(rootPath + '/lib/download.dex')
    try {
  ...
  this.loadReleaseInfo = function () {
    ...
    let loadingDialog = dialogs.build({
      ...
      .show()
    let summary = _this.downloader.getUpdateSummary()
    if (summary === null) {
    ...
    let localVersion = _this.downloader.getLocalVersion()
