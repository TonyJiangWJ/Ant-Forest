 ...
 * @Last Modified time: 2020-09-22 20:40:03
 * @Description: 
 */
...
function CommonAutomation () {
  ...
  this.enterFriendList = function (tryCount) {
    tryCount = tryCount || 1
    if (descMatches(_config.enter_friend_list_ui_content).exists()) {
      descMatches(_config.enter_friend_list_ui_content)
        .findOne(_config.timeout_findOne)
        .click()
    } else if (textMatches(_config.enter_friend_list_ui_content).exists()) {
      textMatches(_config.enter_friend_list_ui_content)
        .findOne(_config.timeout_findOne)
        .click()
    } else {
      if (tryCount > 3) {
      ...
      _logUtils.warnInfo(['未找到 {} 等待一秒钟后重试, 尝试次数：{}', _config.enter_friend_list_ui_content, tryCount])
      // 未找到查看更多好友，等待1秒钟后重试
      sleep(1000)
