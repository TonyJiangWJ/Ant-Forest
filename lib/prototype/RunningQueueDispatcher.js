...
function RunningQueueDispatcher () {
  ...
  this.getRunningStatus = function (quite) {
    ...
    if (storedRunningTask) {
      ...
      if (currentTimestamp > runningTask.timeout) {
        !quite && _logUtils.debugInfo('运行中任务已超时：' + storedRunningTask + ' 超时时间：' + ((currentTimestamp - runningTask.timeout) / 1000).toFixed(0) + '秒')
        // 直接移除已超时运行中的任务
        this.getStorage().put(RUNNING_KEY, '')
        return null
      } else {
        !quite && _logUtils.debugInfo('获取运行中任务信息：' + storedRunningTask + ' 超时剩余时间：' + ((runningTask.timeout - currentTimestamp) / 1000).toFixed(0) + '秒')
        return runningTask
      }
    } else {
      return null
   ...
   * @returns 
   */
  this.checkAnyDelayInfo = function () {
    let delayInfoStr = this.getStorage().get(DELAY_KEY)
    if (delayInfoStr) {
      let delayInfo = JSON.parse(delayInfoStr)
      let currentTimestamp = new Date().getTime()
      if (currentTimestamp > delayInfo.timeout) {
        _logUtils.debugInfo('等待队列已超时：超时时间：' + ((currentTimestamp - delayInfo.timeout) / 1000).toFixed(0) + '秒')
        return false
      } else {
        _logUtils.debugInfo('获取运行中 当前调度等待中，超时剩余时间：' + ((delayInfo.timeout - currentTimestamp) / 1000).toFixed(0) + '秒')
        return true
      }
    } else {
      return false
  ...
  /**
   * 设置延迟信息 标记五分钟内的延迟信息，使后续定时触发脚本都先自动加入等待队列
   */
  this.setupDelayInfo = function () {
    let taskInfo = this.getCurrentTaskInfo()
    let delayInfo = {
      source: taskInfo.source,
      // 设置超时时间为5分钟
    ...
    if (this.lock()) {
      this.getStorage().put(DELAY_KEY, JSON.stringify(delayInfo))
      this.unlock()
    }
  }
   ...
   * 清除延迟信息
   */
  this.clearDelayInfo = function () {
    // 无需加锁 直接清除等待信息
    this.getStorage().put(DELAY_KEY, '')
  }

  this.addRunningTask = function (cutInQueue) {
    if (typeof cutInQueue === 'undefined') {
      // 直接手动触发的，直接插队
      cutInQueue = !executedIntent
    }
    let taskInfo = this.getCurrentTaskInfo()
    // 非通知运行或者非插队运行的 且存在延迟信息 直接加入等待队列
    if (!config._triggerByNotice && !cutInQueue && this.checkAnyDelayInfo()) {
      _logUtils.debugInfo('当前存在延迟运行信息，将当前task放入等待队列：' + JSON.stringify(taskInfo))
   ...
   * @param {boolean} keepRunning 是否保持运行 直到获取锁 默认false
   * @param {boolean} quite 获取runningTask时是否静默 不打印日志信息
   */
  this.renewalRunningTask = function (time, keepRunning, quite) {
    time = time || 15
    let taskInfo = this.getCurrentTaskInfo()
    let runningTask = this.getRunningStatus(quite)
    if (runningTask) {
