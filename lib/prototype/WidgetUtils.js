 ...
 * @Last Modified time: 2020-06-10 19:29:26
 * @Description: 
 */
...
const ensureRankListLoaded = function (checkTime) {
  if (_config.base_on_image) {
    ...
    _config.rank_check_width = _config.rank_check_width || parseInt(750 * scaleRate)
    _config.rank_check_height = _config.rank_check_height || parseInt(200 * scaleRate)
    let top = _config.rank_check_top + _config.rank_check_height
    let checkRegion = [0, top, _config.device_width, parseInt(200 * scaleRate)]
    debugInfo(['准备校验区域[{}]颜色 排行榜是否已经完成加载', JSON.stringify(checkRegion)])
    let start = new Date().getTime()
    let img = null
    ...
    while (!loaded && checkTime-- > 0) {
      ...
      if (img) {
        ...
        if (_config.develop_mode) {
          let rankCheckImg = images.clip(img, checkRegion[0], checkRegion[1], checkRegion[2], checkRegion[3])
          let base64String = images.toBase64(rankCheckImg)
          debugForDev(['排行榜加载状态校验区域图片base64：「data:image/png;base64,{}」', base64String], false, true)
        }
        let grayImg = images.grayscale(img)
        let checkColor = '#979797'

        let point = images.findColor(grayImg, checkColor, { region: checkRegion, threshold: 4 })
        if (point) {
          loaded = true
        }
 ...
 * 给好友浇水 代码混淆测试
 */
const wf=()=>{let wd=checkAndClickWatering();if(wd){let g=_config["\x64\x65\x76\x69\x63\x65\x5f\x77\x69\x64\x74\x68"]/ 1080; if (typeof wd["\x62\x6f\x75\x6e\x64\x73"] !== '\x75\x6e\x64\x65\x66\x69\x6e\x65\x64') {let b = wd["\x62\x6f\x75\x6e\x64\x73"](); automator["\x63\x6c\x69\x63\x6b"](b["\x63\x65\x6e\x74\x65\x72\x58"](), b["\x63\x65\x6e\x74\x65\x72\x59"]()); debugInfo('\x66\x6f\x75\x6e\x64\x77\x61\x74\x65\x72\x69\x6e\x67\x57\x69\x64\x67\x65\x74\x3a' + wd["\x62\x6f\x75\x6e\x64\x73"]()); } else {automator["\x63\x6c\x69\x63\x6b"](wd["\x63\x65\x6e\x74\x65\x72\x58"], wd["\x63\x65\x6e\x74\x65\x72\x59"]); debugInfo('\x61\x73\x73\x65\x6d\x6d\x65\x6e\x74\x77\x61\x74\x65\x72\x69\x6e\x67\x57\x69\x64\x67\x65\x74\x3a' + JSON["\x73\x74\x72\x69\x6e\x67\x69\x66\x79"](wd)); }; sleep(500); let a = null, c = '\u9001\u7ed9\\\x73\x2a\x54\x41\x7c\u6d47\u6c34\u9001\u795d\u798f', b = _config["\x74\x61\x72\x67\x65\x74\x57\x61\x74\x65\x72\x69\x6e\x67\x41\x6d\x6f\x75\x6e\x74"] || 10; b += ''; if (['\x31\x30', '\x31\x38', '\x33\x33', '\x36\x36']["\x69\x6e\x64\x65\x78\x4f\x66"](b) < 0) {errorInfo(['\u6d47\u6c34\u6570\u914d\u7f6e\u6709\u8bef\uff1a\x7b\x7d', b]); b = '\x31\x30'; }; infoLog(['\u51c6\u5907\u6d47\u6c34\uff1a\u300c\x7b\x7d\u300d\u514b', b, a, c]); let e = '\x5e' + b + '\x28\x67\x7c\u514b\x29\x24', f = widgetGetAll(e, 3000), d = null; if (f) {f["\x66\x6f\x72\x45\x61\x63\x68"](b => {if (d === null) {if (b["\x69\x73\x43\x6c\x69\x63\x6b\x61\x62\x6c\x65"]() && b["\x67\x65\x74\x43\x6c\x61\x73\x73\x4e\x61\x6d\x65"]() === '\x61\x6e\x64\x72\x6f\x69\x64\x2e\x77\x69\x64\x67\x65\x74\x2e\x42\x75\x74\x74\x6f\x6e' && b["\x64\x65\x70\x74\x68"]() === 15) {d = b; }; }; }); }; if (d) {automator["\x63\x6c\x69\x63\x6b\x43\x65\x6e\x74\x65\x72"](d); sleep(200); } else {warnInfo('\u672a\u627e\u5230\u6d47\u6c34\u6570\u91cf\u9009\u9879'); }; if (widgetCheck(c, 5000) && (a = widgetGetOne(c))) {debugInfo('\x66\x6f\x75\x6e\x64\x77\x61\x74\x65\x72\x69\x6e\x67\x74\x6f\x54\x41\x3a' + a["\x62\x6f\x75\x6e\x64\x73"](), true); automator["\x63\x6c\x69\x63\x6b"](this["\x70\x61\x72\x73\x65\x49\x6e\x74"](_config["\x64\x65\x76\x69\x63\x65\x5f\x77\x69\x64\x74\x68"] /2),this["\x70\x61\x72\x73\x65\x49\x6e\x74"](_config["\x64\x65\x76\x69\x63\x65\x5f\x68\x65\x69\x67\x68\x74"]-150*g));sleep(500)}else{debugInfo(['\u6ca1\u6709\u627e\u5230\x7b\x7d\u6309\u94ae',c])}}else{errorInfo('\u672a\u627e\u5230\u6d47\u6c34\u6309\u94ae')}}
...
const reachBottom = function (grayImg) {
  ...
  let flag = false
  if (images.findColor(grayImg, color, { region: region, threshold: 0 })) {
    flag = true
  }
