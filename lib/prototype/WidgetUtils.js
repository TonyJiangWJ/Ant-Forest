 ...
 * @Last Modified time: 2020-07-30 20:38:17
 * @Description: 
 */
...
const ensureRankListLoaded = function (checkTime) {
  if (_config.base_on_image) {
    let start = new Date().getTime()
    let loaded = checkIsInFriendListByImg(checkTime)
...
const tryFindBottomRegion = function (grayImg) {
  ...
  if (!_config.bottom_check_succeed) {
    let detectRegion = [parseInt(_config.device_width * 0.6), parseInt(_config.device_height * 0.8), parseInt(_config.device_width - 100 - _config.device_width * 0.6), parseInt(_config.device_height * 0.19)]
