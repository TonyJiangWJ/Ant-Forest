 ...
 * @Last Modified time: 2020-10-26 21:33:57
 * @Description: 
 */
 ...
 */
const alternativeWidget = function (contentA, contentB, timeout) {
  ...
  let descThreadA = threads.start(function () {
    ...
    let res = descMatches(matchRegexA).findOne().desc()
  ...
  let textThreadA = threads.start(function () {
    ...
    let res = textMatches(matchRegexA).findOne().text()
  ...
  let descThreadB = threads.start(function () {
    ...
    let res = descMatches(matchRegexB).findOne().desc()
  ...
  let textThreadB = threads.start(function () {
    ...
    let res = textMatches(matchRegexB).findOne().text()
  ...
  // 超时返回0 找到A返回1 否则返回2
  return timeoutFlag ? 0 : (findA ? 1 : 2)
}
