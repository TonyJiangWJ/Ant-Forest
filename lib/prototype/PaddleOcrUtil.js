...
PaddleOcrUtil.prototype.init = function () {
  if (typeof $ocr !== 'undefined') {
    LogUtils.debugInfo(['使用内置$OCR'])
    this.initialized = true
    this.internalSupport = true
    this.enabled = true
    return true
  }
  try {
 ...
 */
 PaddleOcrUtil.prototype.ensureModelLoaded = function () {
  let _this = this
  let ocrResults = util.java.toJsArray(_this.predictor.runOcr(images.fromBase64(check_model_base64).getBitmap()))
  ...
  threads.start(function () {
    while (checkResult != '测试' && limit-- > 0) {
      ...
      ocrResults = util.java.toJsArray(_this.predictor.runOcr(images.fromBase64(check_model_base64).getBitmap()))
  ...
  if (loading.blockedGet() == true) {
    commonFunctions.registerOnEngineRemoved(function () {
      _this.predictor.releaseModel()
    }, 'release paddle model')
  }
}
...
PaddleOcrUtil.prototype.recognize = function (img, region) {
  if (this.internalSupport) {
    return $ocr.recognizeText(img, { region: region }).join('\n')
  }
  if (!this.initialized) {
    this.init()
  }
  if (this.enabled) {
    ...
    start = new Date()
    let ocrResults = util.java.toJsArray(this.predictor.runOcr(bitmap)).filter(v => v.confidence > 0.5)
 ...
 */
PaddleOcrUtil.prototype.recognizeWithBounds = function (img, region, regex, showOcrDetail) {
  if (this.internalSupport) {
    let result = $ocr.detect(img, { region: region }).filter(r => r.confidence > 0.5)
    if (regex) {
  ...
  if (this.enabled) {
    ...
    LogUtils.debugInfo(['图片转换耗时：{}ms', new Date() - start])
    start = new Date()
    let result = util.java.toJsArray(this.predictor.runOcr(bitmap)).filter(v => v.confidence > 0.5)
    LogUtils.debugInfo(['paddle识别文本耗时：{}ms', new Date() - start])
    if (showOcrDetail) {
      LogUtils.debugInfo(['paddle识别文本信息：{}', JSON.stringify(result)])
    }
    if (regex) {
      regex = new RegExp(regex)
      result = result.filter(r => regex.test(r.label))
