...
PaddleOcrUtil.prototype.recognize = function (img, region) {
  if (this.enabled) {
    ...
    let start = new Date()
    let ocrResults = runtime.bridges.bridges.toArray(this.predictor.runOcr(img.getBitmap())).filter(v => v.confidence > 0.5)
...
PaddleOcrUtil.prototype.recognizeWithBounds = function (img, region, regex) {
  if (this.enabled) {
    ...
    let start = new Date()
    let result = runtime.bridges.bridges.toArray(this.predictor.runOcr(img.getBitmap())).filter(v => v.confidence > 0.5)
    LogUtils.debugInfo(['识别文本耗时：{}ms', new Date() - start])
    if (regex) {
