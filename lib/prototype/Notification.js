 ...
 * @Last Modified time: 2025-04-08 12:45:33
...
function NotificationHelper () {
    ...
    this.cancelNotice = function (notificationId, offset) {
        let notificationManager = context.getSystemService(Context.NOTIFICATION_SERVICE)
        notificationManager.cancel(getNotificationId(notificationId, offset));
    }

    function getNotificationId(notificationId, offset) {
        offset = offset || 10
        if (typeof notificationId == 'string') {
            return NOTIFICATION_ID * offset | simpleHash(notificationId)
        }
        notificationId = notificationId || (NOTIFICATION_ID * offset | simpleHash(engines.myEngine().source))
