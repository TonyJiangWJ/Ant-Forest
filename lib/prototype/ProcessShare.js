...
function ProcessShare () {
   ...
   * @param {number} timeout  订阅超时时间，超时后自动停止线程 默认60秒
  ...
  this.subscribe = function (resolve, timeout, filePath) {
    ...
    let subscriber = ProcessMappedShare.newSubscriber(filePath, this.bufferSize, runtime).setLoop(this.isLoop).timeout(timeout)
