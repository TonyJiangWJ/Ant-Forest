...
function checkYoloModelValid (modelPath) {
  ...
  let modelLastLine = readLastLine(modelPath)
  if (!modelLastLine) {
    warnInfo(['yolo模型文件为空，请检查模型文件'])
    yoloSupport = false
    return null
  }
  console.verbose('model last line:', modelLastLine)
  let regex = /^.*names.*(\{.*\})/
