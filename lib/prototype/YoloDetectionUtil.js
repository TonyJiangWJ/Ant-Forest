...
if (config.detect_by_yolo && yoloSupport && !files.exists(modelPath)) {
  warnInfo(['yolo预训练模型不存在，请通过[{}]下载模型，或者手动下载[{}]并放置以下路径：{}', 'unit/模型下载.js', config.yolo_onnx_model_url, modelPath])
  yoloSupport = false
}


function YoloDetection () {
  this.labels = checkYoloModelValid(modelPath)
  this.enabled = yoloSupport && config.detect_by_yolo && !!this.labels
  if (yoloSupport && this.labels) {
  ...
  this.forward = function (img, filterOption) {
    ...
    if (filterOption.showYoloDetail) {
      debugInfo(['yolo detect result: {}', JSON.stringify(resultList)])
    } else {
...
function checkYoloModelValid (modelPath) {
  if (yoloSupport == false || !config.detect_by_yolo) {
    return null
  }
  let metaNames
  try {
    ...
    errorInfo(['yolo模型文件格式不正确，请检查模型文件' + e], true)
    return null
  }
