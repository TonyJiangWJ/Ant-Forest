 ...
 * @Last Modified time: 2020-09-22 20:40:12
 * @Description: 通用工具
 */
...
function CommonFunctions () {
  ...
  */
  ...
  this.getDistanceAndGravity = function (time) {
    ...
    let distanceCount = 0
    let totalDistance = 0
    sensors.register("proximity", sensors.delay.normal)
      .on("change", (event, d) => {
        _logUtils.debugInfo(util.format("当前距离: %d", d))
        totalDistance += d
        distanceCount++
      })
    ...
    _logUtils.debugInfo(util.format('平均重力加速度：%d %d %d 平均距离：%d', result.ax, result.ay, result.az, (totalDistance / distanceCount)))
  ...
  this.inLimitTimeRange = function () {
    ...
    return !_config.develop_mode && 30 <= checkValue && 650 >= checkValue
  }
}
