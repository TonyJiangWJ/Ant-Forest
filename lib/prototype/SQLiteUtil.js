...
function SQLiteUtil () {
   ...
   * 遍历表定义，获取当前最大版本号
   *
   * @param {Array} tables 表定义列表
   * @returns 
   */
  function findMaxVersion (tables) {
    let tableVersion = tables.filter(table => !!table.version).reduce((max, table) => Math.max(max, table.version), 0)
    let alertVersion = tables.map(table => table.tableAlters)
      .map(tableAlters => tableAlters.map(alter => alter.version)
        .filter(alterVersion => !!alterVersion)
        .reduce((max, alterVersion) => Math.max(max, alterVersion), 0))
      .reduce((max, alterVersion) => Math.max(max, alterVersion), 0)
    return Math.max(tableVersion, alertVersion)
  }
  ...
  this.initDbTables = function (fileName, tables, version) {
    if (typeof version == 'undefined' || version == null) {
      version = findMaxVersion(tables)
      debugInfo(['自动计算当前数据库版本号为：{}', version])
    }
    version = version || 1
    ...
    let alterSqls = tables.map(table => {
       ...
       * 
       * table中可以定义version字段，当version大于当前数据库版本号时，执行create和alter语句
       */
      let alters = table.tableAlters || []
      let indexOrOtherAlters = alters.map(sql => {
        if (typeof sql === 'string') {
          if (currentVersion === 0 || table.version > currentVersion) {
            return sql
          }
      ...
      // 对于已存在的数据库文件，创建新的数据库表时，需要指定其version且大于currentVersion时，才会执行：添加到变更sql中
      if (currentVersion > 0 && table.version > currentVersion) {
        return [table.tableCreate].concat(indexOrOtherAlters)
      } else {
        return indexOrOtherAlters
      }
    ...
    this.sqliteHelper = new SQLiteHelper(fileName,
      // 对于新创建的数据库文件，才会执行create语句，已存在的，不会再执行，因此需要正确处理新表的定义
      tables.map(table => new java.lang.String(table.tableCreate)).concat(initSqls),
      alterSqls, version, context)
    let self = this
  ...
  this.query = function (tableName, whereClause, params) {
    ...
    if (resultList != null) {
      return util.java.toJsArray(resultList)
    }
    return []
  }
