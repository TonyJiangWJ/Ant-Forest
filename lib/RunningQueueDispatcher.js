...
function RunningQueueDispatcher () {
  ...
  this.checkDuplicateRunning = function () {
    ...
    debugInfo('Dispatcher:当前脚本信息 id:' + currentEngine.id + ' source:' + currentSource + ' 运行中脚本数量：' + runningSize)
    if (runningSize > 1) {
      runningEngines.forEach(engine => {
        let compareEngine = engine
        let compareSource = compareEngine.getSource() + ''
        debugInfo('Dispatcher:对比脚本信息 id:' + compareEngine.id + ' source:' + compareSource)
        if (currentEngine.id !== compareEngine.id && compareSource === currentSource) {
          warnInfo('Dispatcher:脚本正在运行中 退出当前脚本：' + currentSource, true)
  ...
  this.removeRunningTask = function (checkOwner) {
    ...
    if (runningTask !== null) {
      ...
      if (runningTask.source === taskInfo.source && (!checkOwner || runningTask.engineId === taskInfo.engineId)) {
        ...
        warnInfo('运行中任务：' + JSON.stringify(runningTask) + '和当前任务：' + JSON.stringify(taskInfo) + '不同，不可移除')
      }
    } else {
      warnInfo('无任务在运行中，不可移除')
    }
    // 清空当前任务施加的锁
    this.clearLock()
  }
