...
function scheduler(config) {
  ...
  commonFunctions.log("启动定时：" + formatDate(clockTimeCompare.originDate, "yyyy-MM-dd HH:mm:ss"))

  let nowCompare = new DateCompare(new Date())
  while (nowCompare.compareTo(clockTimeCompare) < 0) {
    nowCompare = new DateCompare(new Date())
    commonFunctions.log("not now " + formatDate(nowCompare.originDate, "yyyy-MM-dd HH:mm:ss"));
    let gap = clockTime.getTime() - nowCompare.getTime();
    if (gap <= 0) {
      break;
    }
    commonFunctions.log("当前时间" + formatDate(new Date(nowCompare.getTime())))
    let leftMinutes = (gap / 60000).toFixed(2)
    commonFunctions.log("剩余时间[" + gap + "]ms [" + leftMinutes + "]分钟 [" + (gap / 3600000).toFixed(2) + "]小时")
    commonFunctions.log("睡眠" + leftMinutes + "分钟")
    commonFunctions.common_delay(leftMinutes, "睡眠还有[")
  }
  commonFunctions.log("start")
}
