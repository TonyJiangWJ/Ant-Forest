...
module.exports = function (BaseHandler) {
  ...
  BaseHandler.executeCollectCycle = () => {
    executeScript('/unit/循环切换小号并收集能量.js')
  }

  BaseHandler.executeRainCycle = () => {
    executeScript('/unit/循环切换小号并执行能量雨收集.js')
  }
  ...
  function queryTargetTimedTaskInfo (targetScriptPath) {
    ...
    if (resultList && resultList.length > 0) {
      return resultList.map(task => {
        ...
        let time = Math.floor(task.getMillis() / 3600000) + ':' + task.getMillis() % 3600000 / 60000
