 ...
 * @Last Modified time: 2020-05-13 00:21:50
...
module.exports = function (__runtime__, scope) {
  if (typeof scope.resourceMonitor === 'undefined' || scope.resourceMonitor === null) {
    ...
    ResourceMonitor.prototype.addImageToList = function (img) {
      ...
      try {
        ...
        if (this.images.length > 50) {
          ...
          // 过滤出超过5秒的图片
          let forRecycle = this.images.filter(imgInfo => current - imgInfo.millis > 5000)
    ...
    function doRecycleImages (forRecycleList, desc) {
      ...
      console.verbose(desc + '，耗时' + (new Date().getTime() - start))
    }
