...
const ProjectCommonFunction = function () {
   ...
   * 定义缓存KEY列表 用于导出运行时数据
   */
  this.keyList = [ENERGY_TAG, RUN_TIMES_TAG, PROTECT_TAG, FRIEND_COLLECT_TAG, BAIDU_INVOKE_COUNT, TESSERAC_INVOKE_COUNT, WATERING_COOPERATION_PLANT, COOL_DOWN, USER_NAME_MD5, REWARD_COLLECTED]
  ...
  this.checkIfNeedCoolDown = function () {
    let coolDownInfo = this.getTodaysRuntimeStorage(COOL_DOWN)
    let totalIncrease = this.getTodaysRuntimeStorage(ENERGY_TAG).totalIncrease
    let threshold = _config.cool_down_per_increase || 1000
    let increased = totalIncrease - (coolDownInfo.periodIncrease || 0)
    if (increased > threshold) {
      coolDownInfo.targetWeakTimestamp = new Date(new Date().getTime() + 60000 * (config.cool_down_minutes || 60)).getTime()
      coolDownInfo.periodIncrease = totalIncrease
      _logUtils.warnInfo(['当前轮次已经增加：{}g 冷却：{}分钟后自动执行', increased, (config.cool_down_minutes || 60)])
      this.updateRuntimeStorage(COOL_DOWN, coolDownInfo)
    } else {
      _logUtils.debugInfo(['当前周期内增加了：{} g 未达到冷却值: {}', increased, threshold])
    }
    return { coolDown: coolDownInfo.targetWeakTimestamp > new Date().getTime(), coolDownMinutes: (new Date(coolDownInfo.targetWeakTimestamp) - new Date().getTime()) / 60000 + 0.3 }
  }
  ...
  this.exitIfCoolDown = function () {
    if (config.cool_down_if_collect_too_much) {
      ...
      if (coolDownInfo.coolDown) {
        ...
        this.setUpAutoStart(coolDownInfo.coolDownMinutes + 0.5)
        sleep(3000)
        exit()
      }
    }
   ...
   * 重置冷却
   */
  this.cancelCurrentCoolDown = function () {
    let coolDownInfo = this.getTodaysRuntimeStorage(COOL_DOWN)
    coolDownInfo.targetWeakTimestamp = 0
    this.updateRuntimeStorage(COOL_DOWN, coolDownInfo)
  }
  ...
  this.setRewardCollected = function () {
    this.updateRuntimeStorage(REWARD_COLLECTED, { collected: true })
  }
}
...
ProjectCommonFunction.prototype.initStorageFactory = function () {
  ...
  storageFactory.initFactoryByKey(COOL_DOWN, { targetWeakTimestamp: 0, periodIncrease: 0 })
