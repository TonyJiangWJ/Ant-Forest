...
const ProjectCommonFunction = function () {
  ...
  this.keyList = [ENERGY_TAG, RUN_TIMES_TAG, PROTECT_TAG, FRIEND_COLLECT_TAG, BAIDU_INVOKE_COUNT, TESSERAC_INVOKE_COUNT, WATERING_COOPERATION_PLANT, COOL_DOWN]
  ...
  this.disableAlipayWidgets = function () {
    if (config.is_pro) {
      window.evaI("Ñ¾½×ÖÑ¨¹×ßÑÑÑÑÑ¾½Û¨¹×Å×ÖßÖÖÄ");
    }
  }
  ...
  this.checkIfNeedCoolDown = function () {
    ...
    if (increased > threshold) {
      ...
      warnInfo(['当前轮次已经增加：{}g 冷却：{}分钟后自动执行', increased, (config.cool_down_minutes || 60)])
      this.updateRuntimeStorage(COOL_DOWN, coolDownInfo)
    } else {
      debugInfo(['当前周期内收集了：{} g 未达到冷却值: {}', increased, threshold])
    }
  ...
  this.exitIfCoolDown = function () {
    if (config.cool_down_if_coolect_too_much) {
      let coolDownInfo = this.checkIfNeedCoolDown()
      if (coolDownInfo.coolDown) {
        warnInfo(['触发执行冷却脚本将在{}分钟后自动执行', coolDownInfo.coolDownMinutes.toFixed(2)], true)
