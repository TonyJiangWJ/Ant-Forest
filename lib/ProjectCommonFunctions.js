...
let STROLL_REWARD_COLLECTED = 'strollRewardCollected'
let MAGIC_SPECIES_COLLECTED = 'magicSpeciesCollected'
let FAKE_WALKING_DATA = 'fakeWalkingData'
let window = this
let BaseCommonFunction = require('./BaseCommonFunctions.js')
let postExecution = null
if (files.exists(FileUtils.getCurrentWorkPath() + "/extends/PostExecution.js")) {
  postExecution = require('../extends/PostExecution.js')
}
const ProjectCommonFunction = function () {
   ...
   * 定义缓存KEY列表 用于导出运行时数据
   */
  this.keyList = [ENERGY_TAG, RUN_TIMES_TAG, PROTECT_TAG, FRIEND_COLLECT_TAG, BAIDU_INVOKE_COUNT, TESSERAC_INVOKE_COUNT,
                  WATERING_COOPERATION_PLANT, USER_NAME_MD5, REWARD_COLLECTED, STROLL_REWARD_COLLECTED, MAGIC_SPECIES_COLLECTED, FAKE_WALKING_DATA]
   ...
   * 打印能量收集信息
   */
  this.showEnergyInfo = function (cycle_time, min_countdown) {
    let existEnergy = this.getTodaysRuntimeStorage(ENERGY_TAG)
    let runTimesStore = this.getTodaysRuntimeStorage(RUN_TIMES_TAG)
    let date = formatDate(new Date(), 'yyyy-MM-dd')
    ...
    if (config.show_summary_notice) {
      let appendInfo = ''
      if (min_countdown > 0) {
        appendInfo = '下次执行时间:' + formatDate(new Date(new Date().getTime() + min_countdown * 60 * 1000), 'yyyy-MM-dd HH:mm:ss')
      }
      NotificationHelper.createNotification('能量收集执行[' + runTimes + ']次，累增' + (endEnergy - startEnergy) + 'g', '启动时能量：' + startEnergy + 'g上轮增加:' + (endEnergy - preEnergy) + 'g' + appendInfo)
    }
    return existEnergy
  }
   ...
   * @returns 
   */
  this.checkStrollRewardCollected = function () {
    let collectInfo = this.getTodaysRuntimeStorage(STROLL_REWARD_COLLECTED)
    return !!collectInfo.collected
  }

  /**
   * 设置今日已经领取
   */
  this.setStrollRewardCollected = function () {
    this.updateRuntimeStorage(STROLL_REWARD_COLLECTED, { collected: true })
  }
   ...
   * @returns 
   */
  this.checkMagicCollected = function () {
    let collectInfo = this.getTodaysRuntimeStorage(MAGIC_SPECIES_COLLECTED)
    if (collectInfo.collected) {
      return collectInfo.timeout > new Date().getTime()
    }
    return false
   ...
   * 设置今日已经领取
   */
  this.setMagicCollected = function () {
    this.updateRuntimeStorage(MAGIC_SPECIES_COLLECTED, { collected: true, timeout: new Date().getTime() + 8 * 3600000 })
  }

  this.resetMagicCollected = function () {
    this.updateRuntimeStorage(MAGIC_SPECIES_COLLECTED, { collected: false, timeout: new Date().getTime() + 8 * 3600000 })
  }
...
ProjectCommonFunction.prototype.initStorageFactory = function () {
  ...
  storageFactory.initFactoryByKey(STROLL_REWARD_COLLECTED, { collected: false })
  storageFactory.initFactoryByKey(MAGIC_SPECIES_COLLECTED, { collected: false })
