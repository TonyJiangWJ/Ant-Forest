...
const ProjectCommonFunction = function () {
  ...
  this.checkIfNeedCoolDown = function () {
    ...
    // 获取1小时内收集的能量总数
    let { increased } = singletonRequire('AntForestDao').getCollectInTimeRange(formatDate(new Date(), 'yyyy-MM-dd'), formatDate(new Date(new Date().getTime() - 3600000)))
    let coolDown = false
    if (increased > threshold) {
      coolDown = true
      _logUtils.warnInfo(['最近半小时内已经收集好友能量：{}g 冷却：{}分钟后自动执行', increased, (config.cool_down_minutes || 60)])
    } else {
      _logUtils.debugInfo(['最近半小时内收集好友能量：{} g 未达到冷却值: {}', increased, threshold])
    }
    return { coolDown: coolDown, coolDownMinutes: config.cool_down_minutes || 60 }
  }
