 ...
 * @Last Modified time: 2019-12-17 21:43:39
 * @Description: 
 */
...
const checkIsInFriendListByImg = function (checkTime) {
  let found = false
  let scaleRate = device.width / 1080
  let checkRegion = [parseInt(170 * scaleRate), parseInt(230 * scaleRate), parseInt(810 * scaleRate), parseInt(100 * scaleRate)]
    ...
    if (img) {
      ...
      found = (() => {
        if (point !== null) {
          let color = 0
          let matchCount = 0
          for (let i = 0; i < 50 * scaleRate; i++) {
            color = img.getBitmap().getPixel(point.x + i, point.y) >> 8 & 0xFF
            if (color === 0xFF) {
              matchCount++
              if (matchCount > 25 * scaleRate) {
                return true
              }
            }
          }
...
const homePageWaiting = function () {
  ...
  if (_config.base_on_image) {
    if (checkIsInFriendListByImg()) {
      errorInfo('错误位置：当前所在位置为好友排行榜')
      return false;
    }
  } else if (idCheck(_config.friend_list_id, 500)) {
    errorInfo('错误位置：当前所在位置为好友排行榜')
    return false;
  }
...
const wateringFriends = function () {
  ...
  if (wateringWidget) {
    ...
    if (widgetCheck(give_content, 5000) && (giveHimButton = widgetGetOne(give_content))) {
      ...
      automator.click(parseInt(device.width / 2), device.height - 100)
      sleep(300)
    } else {
      debugInfo('没有找到 送给TA 按钮')
    }
  } else {
    errorInfo('未找到浇水按钮')
  }
