 ...
 * @Last Modified time: 2019-12-18 19:03:43
 * @Description: 
 */
...
const checkDirectionColors = function (point, count, img) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    color = img.getBitmap().getPixel(point.x, point.y + i) >> 8 & 0xFF
    if (color === 0xFF) {
      matchCount++
    }
    color = img.getBitmap().getPixel(point.x, point.y - i) >> 8 & 0xFF
    ...
    if (img) {
      ...
      found = (() => {
        if (point !== null) {
          ...
          if (!susspendError) {
            errorInfo(['校验排行榜失败，匹配点{}周围匹配的白色点数量：{}', JSON.stringify(point), matchCount])
          }
        }
        return false
      })()
