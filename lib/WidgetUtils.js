 ...
 * @Last Modified time: 2019-12-01 20:50:54
 * @Description: 
 */
...
const foundNoMoreWidget = function (sleepTime) {
  ...
  let noMoreWidget = widgetGetOne(config.no_more_ui_content, sleep, false, true)
  if (noMoreWidget) {
...
const loadFriendList = function () {
  ...
  let foundNoMoreThread = threads.start(function () {
    widgetCheck(config.no_more_ui_content, config.timeoutLoadFriendList || config.timeout_existing)
    timeout = false
    countDown.countDown()
  })
  let timeoutThread = threads.start(function () {
    sleep(config.timeoutLoadFriendList || config.timeout_existing)
    errorInfo("预加载好友列表超时")
    countDown.countDown()
  })
  countDown.await()
...
const getFriendList = function (ownText) {
  ownText = ownText || _own_text
  if (!ownText) {
    _own_text = getOwntext()
    ownText = _own_text
  }
  if (ownText) {
    if (_own_text) {
      debugInfo('已存储个人id：' + _own_text)
    }
    debugInfo('我自己：' + ownText)
    let countdown = new Countdown()
    let self = textMatches(new RegExp('^' + ownText + '$')).findOnce(1)
    if (self) {
      try {
        let rootParent = self.parent().parent().parent().parent()
        debugInfo('列表总长度：' + rootParent.children().length)
        return rootParent
      } catch (e) {
        errorInfo(e)
      } finally {
