...
function CommonFunctions() {
  ...
  this.storeEnergy = function (newVal) {
    ...
    let existEnergy = runtimeStorages.get('energy')
    let stored
    if (existEnergy) {
      existEnergy = JSON.parse(existEnergy)
      let existDate = existEnergy.date
      // 是否是同一天
      if (existDate === today) {
        // 获取已存在的能量值
    ...
    runtimeStorages.put("energy", JSON.stringify(existEnergy))
  }

  this.increaseRunTimes = function () {
    let runtimeStorages = storages.create(RUNTIME_STORAGE)
    let preRunTimes = runtimeStorages.get('runTimes') || 0
    runtimeStorages.put('runTimes', preRunTimes + 1)    
  ...
  this.showEnergyInfo = function () {
    ...
    if (existEnergy) {
      ...
      let runTimes = runtimeStorages.get('runTimes') || 0
      ...
      this.log("energy info is not exist, make sure you have ran the script!")
    }
  }

  this.getEnergyInfo = function () {
    let runtimeStorages = storages.create(RUNTIME_STORAGE)
    let existEnergy = runtimeStorages.get('energy')
    if (existEnergy) {
      return JSON.parse(existEnergy)
    } else {
      return null
    }
  }
