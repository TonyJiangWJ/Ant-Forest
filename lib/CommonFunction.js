...
function CommonFunctions() {
  ...
  this.recordFriendCollectInfo = function (friendCollect) {
    this.debug('保存收集数据' + JSON.stringify(friendCollect))
    ...
    if (existCollectInfoIndex > -1) {
      ...
      collectInfo.todayHelp += collectInfo.helpCollect || 0
      collectInfo.todayWatering += friendCollect.watered ? 10 : 0
    ...
    this.debug('收集后保存收集好友数据：' + JSON.stringify(this.parseFieldToInts(collectInfo)))
    friendCollectStore.collectInfos = existCollectList
    this.updateRuntimeStorage(FRIEND_COLLECT_TAG, friendCollectStore)
    if (collectInfo.todayCollect > config.wateringThresold && collectInfo.todayWatering <= 0) {
      commonFunctions.debug("已收集[" + collectInfo.friendName + "] " + collectInfo.todayCollect + "g 且未浇过水")
      return true
    }
  }
   ...
   * 打印概述信息
   */
  this.showCollectSummary = function () {
    ...
    for (let collectInfo of collectInfos) {
      ...
      log('好友：' + collectInfo.friendName + ' 拥有总能量：' + collectInfo.friendEnergy + 'g')
      ...
      if (collectInfo.todayCollect !== 0) {
        console.info(todayCollectSum + '今日收取ta：' + collectInfo.todayCollect + 'g')
  ...
  this.checkDuplicateRunning = function () {
    ...
    this.debug('当前脚本信息 id:' + currentEngine.id + ' source:' + currentSource + ' 运行中脚本数量：' + runningSize)
    if (runningSize > 1) {
      runningEngines.forEach(script => {
        let compareEngine = script
        let compareSource = compareEngine.getSource() + ''
        this.debug('对比脚本信息 id:' + compareEngine.id + ' source:' + compareSource)
        if (currentEngine.id !== compareEngine.id && compareSource + '' === currentSource) {
          this.warn('脚本正在运行中 退出当前脚本：' + currentSource, true)
