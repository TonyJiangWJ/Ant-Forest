 ...
 * @Last Modified time: 2019-12-03 16:38:01
 * @Description: 通用工具
 */
...
let {
  _logInfo, _debugInfo, _warnInfo, _infoLog, _errorInfo, _appendLog
} = typeof logInfo === 'undefined' ? (() => {
  let { logInfo, debugInfo, warnInfo, infoLog, errorInfo, appendLog } = require('../lib/LogUtils.js')
  debugInfo('commonFunctions重新载入日志方法')
  return {
    _logInfo: logInfo,
...
function CommonFunctions () {
  ...
  this.checkCaptureScreenPermission = function (releaseLock, errorLimit) {
    ...
    do {
      ...
      _debugInfo('获取截图失败 再试一次 count:' + (++errorCount))
    } while (!screen && errorCount < errorLimit)
    if (!screen) {
      _errorInfo(['获取截图失败多次[{}], 可能已经没有了截图权限，重新执行脚本', errorCount], true)
  ...
  this.launchAutoJs = function (reopen) {
    ...
    if (!reopen) {
      let oldTryFlag = config.tryGetExactlyPackage
      let oldTryTime = config.tryGetExactlyPackageTime
      config.tryGetExactlyPackage = true
      config.tryGetExactlyPackageTime = 4
      currentRunning = this.getExactlyCurrentPackage(false)
      config.tryGetExactlyPackage = oldTryFlag
      config.tryGetExactlyPackageTime = oldTryTime
      _debugInfo('启动跳板前所在AppPackage:' + currentRunning)
      beforeSpringboard = currentRunning
      _debugInfo(['记录启动跳板前package：{}', beforeSpringboard])
    }
    ...
    if (currentRunning !== launchPackage) {
      ...
      while (currentRunning !== launchPackage && wait-- > 0) {
        _debugInfo(['未进入AutoJS，继续等待 当前package：{}', currentRunning])
        sleep(1000)
      }
    }
  ...
  this.launchPackage = function (packageName, reopen) {
    _debugInfo(['准备{}打开package: {}', reopen ? '重新' : '', packageName])
    let currentRunning = currentPackage()
    let storedRunningPackage = this.getTodaysRuntimeStorage(RUNNING_PACKAGE)
    let beforeSpringboard = storedRunningPackage.beforeSpringboard
    if (!reopen) {
      _debugInfo(['启动:{}前所在package: {} 已记录的启动跳板前package: {}', packageName, currentRunning, beforeSpringboard])
    }
    ...
    while (currentRunning !== packageName && waitCount-- > 0) {
      _debugInfo(['未进入{}，继续等待 当前所在：{}', packageName, currentRunning])
      sleep(1000)
      currentRunning = currentPackage()
    }
    _debugInfo(['进入[{}] {}', packageName, (packageName === currentRunning ? '成功' : '失败')])
  }
    ...
    threads.start(function () {
      ...
      if (startingPackage !== _this.getAutoJsPackage() && _this.isNotEmpty(beforeSpringboard)) {
        ...
        if (startingPackage.match(/(com.android)|(system)|(home)|(theme)|(autojs)/)) {
          _debugInfo(['读取到的package[{}] 在黑名单中，直接返回', startingPackage])
          back()
          return
        }
        ...
        _debugInfo(['重新打开：{}', startingPackage])
        _this.launchPackage(startingPackage, true)
        sleep(500)
        warningDialog.dismiss()
      } else {
        _debugInfo(['直接返回最小化支付宝'])
        back()
      }
    })
  }
  ...
  this.showPackageBeforeRunning = function () {
    ...
    _debugInfo(['启动跳板前package: {} 开始蚂蚁森林前package: {}', beforeSpringboard, storedRunningPackage.startingPackage])
  ...
  this.getExactlyCurrentPackage = function (reopen) {
    ...
    if (!reopen && config.tryGetExactlyPackage) {
      // miui 经常没法获取到正确的package 打开最近任务并返回多次 可能会拿到
      let tryTime = config.tryGetExactlyPackageTime || 2
  ...
  this.recordCurrentPackage = function () {
    ...
    if (beforeSpringboard) {
      ...
      _debugInfo('维持重新打开的package为跳板前数据：' + beforeSpringboard)
    } else {
      sleep(500)
      let currentRunning = this.getExactlyCurrentPackage()
      _debugInfo(['启动前所在package：{} 已记录的跳板前package：{}', currentRunning, beforeSpringboard])
      this.getAndUpdateSpringboard(beforeSpringboard)
      this.getAndUpdateBeforeStarting(currentRunning)
      _debugInfo('更新重新打开的package为当前启动前数据：' + currentRunning)
    }
  }
  ...
  this.showDialogAndWait = function (checkedSpringboard) {
    if (checkedSpringboard) {
      ...
      if (storedRunningPackage.beforeSpringboard) {
        _debugInfo(['跳转跳板已经提示过，不再展示延迟对话框，跳板前package：{}', storedRunningPackage.beforeSpringboard])
        return
      }
    } else {
  ...
  this.showMiniFloaty = function (text, x, y, color) {
    _FloatyInstance.setFloatyInfo({ x: x || config.min_floaty_x || 150, y: y || config.min_floaty_y || 20 }, text)
    _FloatyInstance.setFloatyTextColor(color || config.min_floaty_color || '#00FF00')
  }
  ...
  this.listenDelayCollect = function () {
    ...
    threads.start(function () {
      sleep(2000)
      _infoLog('即将收取能量，按音量下键延迟五分钟执行', true)
      _debugInfo('after setMaxListeners')
      events.observeKey()
      _debugInfo('after observeKey')
      events.onceKeyDown('volume_down', function (event) {
        if (config.autoSetBrightness) {
          device.setBrightnessMode(1)
        }
        _warnInfo('延迟五分钟后启动脚本', true)
        _this.setUpAutoStart(5)
        engines.myEngine().forceStop()
        _runningQueueDispatcher.removeRunningTask()
        exit()
      })
      _debugInfo('after setOnceKeyDown')
    })
  }

  this.commonDelay = function (minutes, text) {
    _debugInfo('倒计时' + minutes)
  ...
  this.getTodaysRuntimeStorage = function (key) {
    ...
    if (existStoreObjStr) {
      try {
        ...
        _debugInfo(["解析JSON数据失败, key:{} value:{} error:{}", key, existStoreObjStr, e])
      }
    }
  ...
  this.recordFriendCollectInfo = function (friendCollect) {
    _debugInfo('保存收集数据' + JSON.stringify(friendCollect))
    ...
    if (
      ...
      _infoLog("已收集[" + collectInfo.friendName + "] " + collectInfo.todayCollect + "g 给他浇浇水")
    ...
    _debugInfo('收集后保存收集好友数据：' + JSON.stringify(this.parseFieldToInts(collectInfo)))
  ...
  this.checkDuplicateRunning = function () {
    ...
    _debugInfo('当前脚本信息 id:' + currentEngine.id + ' source:' + currentSource + ' 运行中脚本数量：' + runningSize)
    if (runningSize > 1) {
      runningEngines.forEach(engine => {
        let compareEngine = engine
        let compareSource = compareEngine.getSource() + ''
        _debugInfo('对比脚本信息 id:' + compareEngine.id + ' source:' + compareSource)
        if (currentEngine.id !== compareEngine.id && compareSource === currentSource) {
          _warnInfo('脚本正在运行中 退出当前脚本：' + currentSource, true)
          _runningQueueDispatcher.removeRunningTask(true)
          engines.myEngine().forceStop()
          exit()
        }
    ...
    if (runningSize > 1) {
      runningEngines.forEach(engine => {
        ...
        if (compareSource === mainScriptJs) {
          _warnInfo(['关闭运行中脚本：id[{}]', compareEngine.id], true)
          engine.forceStop()
        }
      })
    }
  ...
  this.setUpAutoStart = function (minutes) {
    ...
    _infoLog('预订[' + minutes + ']分钟后的任务，时间戳:' + millis)
    // 预定一个{minutes}分钟后的任务
    let task = Timers.addDisposableTask({
      path: mainScriptJs,
      date: millis
    })
    _debugInfo("定时任务预定成功: " + task.id)
  ...
  this.cancelAllTimedTasks = function () {
    ...
    if (autoStartListStr) {
      ...
      if (array && array.length > 0) {
        array.forEach(task => {
          _debugInfo('撤销自动任务：' + JSON.stringify(task))
          if (task.mId) {
