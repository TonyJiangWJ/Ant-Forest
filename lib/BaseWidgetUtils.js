...
const BaseWidgetUtils = function () {
   ...
   */
  this.widgetGetOne = function (contentVal, timeout, containType, suspendWarning, appendFilter) {
    ...
    if (!checkResult.timeout) {
      ...
      if (!checkResult.isDesc) {
        target = appendFilter(textMatches(matchRegex)).findOne(_config.timeout_findOne)
      } else {
        isDesc = true
        target = appendFilter(descMatches(matchRegex)).findOne(_config.timeout_findOne)
      }
    }
   ...
   * @param {function} appendFilter 附加查询条件 详见UiSelector
   */
  this.widgetGetAll = function (contentVal, timeout, containType, appendFilter) {
    appendFilter = appendFilter || function (matcher) { return matcher }
    let target = null
    let isDesc = false
    let timeoutFlag = true
    ...
    if (!checkResult.timeout) {
      ...
      let matchRegex = new RegExp(contentVal)
      if (!checkResult.isDesc) {
        target = appendFilter(textMatches(matchRegex)).untilFind()
      } else {
        isDesc = true
        target = appendFilter(descMatches(matchRegex)).untilFind()
      }
    }
   ...
   * 超时返回false
   */
  this.subWidgetGetOne = function (container, contentVal, timeout, containType, appendFilter) {
    appendFilter = appendFilter || function (matcher) { return matcher }
    timeout = timeout || _config.timeout_findOne
    let countDown = new java.util.concurrent.CountDownLatch(1)
    let matchRegex = new RegExp(contentVal)
    ...
    let target = null
    let descThread = threads.start(function () {
      let descTarget = appendFilter(descMatches(matchRegex)).findOneOf(container)
      if (descTarget) {
    ...
    let textThread = threads.start(function () {
      let textTarget = appendFilter(textMatches(matchRegex)).findOneOf(container)
      if (textTarget) {
   ...
   * 超时返回false
   */
  this.subWidgetGetAll = function (container, contentVal, timeout, containType, appendFilter) {
    appendFilter = appendFilter || function (matcher) { return matcher }
    let exists = this.subWidgetGetOne(container, contentVal, timeout, containType, appendFilter)
    if (exists) {
      let matchRegex = new RegExp(contentVal)
      let resultList = []
      if (exists.isDesc) {
        resultList = appendFilter(descMatches(matchRegex)).findOf(container)
      } else {
        resultList = appendFilter(textMatches(matchRegex)).findOf(container)
      }
      if (containType) {
        return {
          target: resultList,
