...
const BaseWidgetUtils = function () {
   ...
   * 超时返回false
   */
   this.subWidgetGetOne = function (container, contentVal, containType) {
    let countDown = new java.util.concurrent.CountDownLatch(1)
    let matchRegex = new RegExp(contentVal)
    let isDesc = false
    let isText = false
    let target = null
    let descThread = threads.start(function () {
      let descTarget = descMatches(matchRegex).findOneOf(container)
      if (descTarget) {
    ...
    let textThread = threads.start(function () {
      let textTarget = textMatches(matchRegex).findOneOf(container)
      if (textTarget) {
   ...
   * 超时返回false
   */
   this.subWidgetGetAll = function (container, contentVal, containType) {
    let exists = this.subWidgetGetOne(container, contentVal, containType)
    if (exists) {
      let matchRegex = new RegExp(contentVal)
      let resultList = []
      if (exists.isDesc) {
        resultList = descMatches(matchRegex).findOf(container)
      } else {
        resultList = textMatches(matchRegex).findOf(container)
      }
      if (containType) {
        return {
          target: resultList,
