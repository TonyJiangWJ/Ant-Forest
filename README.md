...
- 默认配置下已开启基于图像识别的方式来执行，好友数量较少的（100左右）可以直接通过控件分析就行，但是好友数量较多的基于控件分析的会特别卡顿需要开启基于图像分析，具体参考下述配置说明
...
- 直接进入排行榜后运行 `test/MockDetect.js` 得到具体像素点个数，分析倒计时和小手的个数差异，设置为他们的一个区分点，比如大多数倒计时像素点个数大于2400，而小手像素点个数小于2400，则设置 `小手像素点个数` 为2400即可。
  ![获取像素点个数](./resources/mock_detect.jpg)
