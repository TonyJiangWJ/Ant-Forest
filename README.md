...
- 在运行可视化配置.js之后进入 `进阶配置` 直接点击 `实时查看可视化配置信息` 按钮来刷新配置并显示当前的框选区域。也可以在配置完之后可以运行`test/全局悬浮窗显示-配置信息.js` 查看配置生效状况，可在 `resources` 目录下截图保存 `region_check.jpg` ，然后可以悬浮窗显示半透明信息，方便在运行 `可视化配置.js` 时拖动进度条快速调整具体区域
...
- 增加了自动收集能量雨的脚本。运行 `unit/能量雨收集.js` 然后打开能量雨界面，并手动开始，点击开始脚本会自动识别并点击
- 除非手机卡了否则不太会漏，基本每天第一次能保证在110g以上 第二次在55g以上
- ![预览](https://user-images.githubusercontent.com/11325805/122257400-90228e00-cf02-11eb-89f0-43f60a8da105.gif)
