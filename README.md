...
* **核心依赖**: 本脚本基于 **[AutoJS Modify](https://github.com/TonyJiangWJ/Ant-Forest/releases/download/v1.1.1.4/AutoJS.modify.latest.apk)** 或 **[困鱼 (AutoJS.fake)](https://github.com/TonyJiangWJ/Ant-Forest/releases/download/v1.1.1.4/AutoJS.fake.latest.apk)** 开发和运行。建议直接使用困鱼，因为目前市面上大部分应用检测autojs关键字，会导致黑号。
...
* **配置**:
  ...
  * 开启TTS语音提醒：默认关闭，需在可视化配置-能量雨设置中开启。开启后可以在脚本遇到人机验证时，触发TTS语音提醒人工介入。
* **效果**: 暴力点击，基本保证满分。

### 拼手速脚本

* **手动启动**: 运行 `unit/拼手速.js` 然后进入打地鼠攒能量界面。等倒计时开始时手动点击开始，脚本会自动点击9秒，基本拿满15次。
* **配置**:
  * 首次运行时，请先截图打地鼠界面，在相册中根据截图来配置具体点击的位置。点击 `更改点击区域` 即可通过拖动条控制点击的位置，操作简单不赘述，自行摸索即可。
  * 演示视频：https://b23.tv/Zazmcwc
* **效果**: 暴力点击，基本保证15次拿满。

### 森林寻宝

* **手动启动**: 运行 `unit/森林寻宝.js` 点击 `自动获取并执行任务` 将通过接口获取互助码，自动给其他人助力获取抽奖机会。
* **自动运行**: 给该脚本设置每日两次的定时任务，避免第一次运行时，互助任务未完成。当定时任务触发时，将自动互助、领取奖励、执行逛一逛、自动消耗抽奖。
* **配置**:
  * 无需特别配置，但是可以分享自己的互助码，点击分享任务，分享到微信，将自动复制互助码，然后通过 `上传新的互助码` 将互助码上传即可。也可以在 `可视化配置` - `寻宝助力` 中上传或者获取互助码。可以查看当日被获取的次数、剩余有效期等。如果当日被报告无效次数大于0，建议更新互助码，当达到3次，将自动被删除。
  * 注意互助码目前有效期是一个月，且在当前活动期内有效。请自己注意更新互助码。另外如果你每日活跃使用，你的互助码将会优先被获取，当前逻辑是活跃用户的互助码每日保底被获取3次。
  * 当前寻宝会有两个活动，可以根据上传界面的提示上传两份，但是请不要搞混淆，避免给他人带来麻烦。
* **效果**: 每天大概能获取13次的抽奖机会，完成当期任务完全没问题。
...
* **报错 `获取截图失败多次`**:
  * 在 `可视化配置` -> `高级设置` 中，尝试增加 `获取截图等待时间` (默认 500ms)，或取消勾选 `是否异步等待截图`（但是脚本可能会一直卡死，这个是autojs底层的bug，目前无合适解决方案）。
* **定时任务不准时**:
