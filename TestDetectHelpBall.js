...
function drawCoordinateAxis (canvas, paint) {
  ...
  paint.setStyle(Paint.Style.FILL)
  paint.setTextSize(10)
  let colorVal = colors.parseColor('#000000')
...
let detectThread = threads.start(function () {
  while (true) {
    ...
    if (screen) {
      ...
      if (new Date().getTime() - birthTime > 900) {

        intervalImg = images.inRange(copyImg, '#b2b2b2', '#d6d6d6')
        if (flag == 1) {
        ...
        colorInP = colors.toString(copyImg.getBitmap().getPixel(225, 725))
  ...
  try {
    ...
    if (clickPoints && clickPoints.length > 0) {
      ...
      clickPoints.forEach((p) => {
        ...
        drawRectAndText('', [p.x + 150 + 25 - 2, p.y + 500 - 2, 4, 4], '#ff00ff', canvas, paint)
        drawRectAndText('', [p.x + 150 - 2, p.y + 500 + 25 - 2, 4, 4], '#ff00ff', canvas, paint)
      })
    }
